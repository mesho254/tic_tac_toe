...
module overmind::tic_tac_toe {
    ...
    */
    fun init_module(admin: &signer) {
        ...
        let resource_account_address = Signer::address_of(admin) + vector<u8>::from_hex(HEX_SEED);

        // TODO: Register the resource account with the the AptosCoin
        let account_capability: R#AccountCapability = R#AccountCapability::new(move(resource_account_address));
        let account_capability_path = AptosCoin::account_capability_path_for(*admin);
        AptosCoin::add_account_capability(account_capability_path, account_capability);

        // TODO: Create the State resource and move it to the resource account
        let state = State {
            games: Vector::empty(),
        };
        move_to(state, &mut resource_account_address);
    ...
    */
    public entry fun create_game(
        ...
        check_if_user_has_enough_apt_coin(game_creator, PRIZE_AMOUNT_APT);
        ...
        let resource_account_address = Signer::address_of(game_creator) + vector<u8>::from_hex(HEX_SEED);
        let transfer_capability_path = AptosCoin::account_capability_path_for(*game_creator);
        AptosCoin::transfer_capability(transfer_capability_path, resource_account_address, PRIZE_AMOUNT_APT);
        ...
        let mut state: &mut R#State = &mut move_to(account_of<State>(resource_account_address));
        ...
        let new_game = Game {
            game_id: state.game_counter,
            player_one: player_one_address,
            player_two: player_two_address,
            current_turn: PLAYER_ONE,
            expiration_timestamp_seconds: (Env::block_timestamp() + DURATION_GAME_START),
            spaces: Vector::fill_with(3, Vector::fill_with(3, None)),
            result: None,
        };
        state.games.push_back(new_game);

        // TODO: Emit a GameCreatedEvent
        emit GameCreatedEvent(player_one_address, player_two_address, state.game_counter);
    }
    ...
    */ 
    public entry fun mark_space(
        ...
        check_if_game_id_is_valid(game_id);


        // TODO: Ensure `player` is a player in the game
        // 
        // HINT: Use the check_if_user_is_player function
        let state_ref = &mut get_account::<State>(account_of<Signer>(Signer::address_of(player)));
        let game = get_game_by_id(state_ref, game_id);
        check_if_user_is_player(player, &game);
        ...
        check_if_it_is_players_turn(player, &game);
        ...
        // HINT: Use the check_if_space_is_open function
         check_if_space_is_open(row_index, column_index, &game);

        // TODO: Fill the specified game space with the correct value (according to consts defined 
        //          above)
        let mark = if player == &game.player_one { PLAYER_ONE_MARK } else { PLAYER_TWO_MARK };
        let updated_game = mark_game_space(game, row_index as usize, column_index as usize, mark);

        // TODO: Emit the GameRoundEvent
        emit GameRoundEvent(player, game_id, row_index, column_index);
        ...
        if player_has_won(updated_game.board, mark, row_index as usize, column_index as usize) {
            let prize_receiver = if player == &game.player_one { &game.player_one } else { &game.player_two };
            let prize_capability_path = AptosCoin::account_capability_path_for(*prize_receiver);
            let transfer_result = AptosCoin::transfer_capability(transfer_capability_path, *prize_receiver, PRIZE_AMOUNT_APT);
            assert(transfer_result, 77); // Transfer failed
            let win_result = if player == &game.player_one { PLAYER_ONE_WON } else { PLAYER_TWO_WON };
            let updated_game_with_result = set_game_result(updated_game, win_result);
            emit GameWonEvent(*prize_receiver, game_id);
            return;
        }
        ...
        if game_spaces_are_full(updated_game.board) {
            let prize_receiver = &game.player_one;
            let prize_capability_path = AptosCoin::account_capability_path_for(*prize_receiver);
            let transfer_result = AptosCoin::transfer_capability(transfer_capability_path, *prize_receiver, PRIZE_AMOUNT_APT);
            assert(transfer_result, 78); // Transfer failed
            let updated_game_with_result = set_game_result(updated_game, GAME_TIED);
            emit GameTiedEvent(game_id);
            return;
        }


        // TODO: Update the game's current turn to the other player
        let next_turn = if player == &game.player_one { PLAYER_TWO } else { PLAYER_ONE };
        let updated_game_with_next_turn = set_game_turn(updated_game, next_turn);


        // TODO: Update the game's expiration timestamp to be now plus DURATION_ROUND
        let updated_game_with_new_expiration = set_game_expiration(updated_game_with_next_turn, (Env::block_timestamp() + DURATION_ROUND));
    }
    ...
    */  
    public entry fun release_funds_after_expiration(game_id: u64) acquires State {
        ...
        let state_ref = &mut get_account::<State>(account_of<State>(state_address()));
        let game = get_game_by_id(state_ref, game_id);
        check_if_game_is_expired(&game);
        // Ensure the prize is unclaimed
        // 
        // HINT: Use the check_if_prize_is_unclaimed function
        check_if_prize_is_unclaimed(&game);
        ...
        let prize_receiver = if game.current_turn == PLAYER_ONE { &game.player_two } else { &game.player_one };
        let prize_capability_path = AptosCoin::account_capability_path_for(*prize_receiver);
        let transfer_result = AptosCoin::transfer_capability(transfer_capability_path, *prize_receiver, PRIZE_AMOUNT_APT);
        assert(transfer_result, 79); // Transfer failed


        // TODO: Fill the game's result with the correct result (according to the GAME_RESULTS 
        //          constants defined above)
        let result = if game.current_turn == PLAYER_ONE { PLAYER_TWO_WON } else { PLAYER_ONE_WON };
        let updated_game_with_result = set_game_result(game, result);

        // TODO: Emit the GameExpiredEvent
        emit GameExpiredEvent(game_id, *prize_receiver);
    ...
    */  
    inline fun row_is_filled_by_player(
        ...
        spaces[row_index as usize]
            .iter()
            .all(|space| match space {
                Some(mark) if mark == player => true,
                _ => false,
            })
    ...
    */  
    inline fun col_is_filled_by_player(
        ...
        //          player and return false otherwise
        spaces.iter()
            .all(|row| match row[column_index as usize] {
                Some(mark) if mark == player => true,
                _ => false,
            })
    }
    ...
    */  
    inline fun game_spaces_are_full(spaces: &vector<vector<Option<u8>>>): bool {
        ...
        spaces.iter().all(|row| row.iter().all(|space| space.is_some()))
    }
    ...
    inline fun check_if_user_has_enough_apt_coin(user: address, amount_of_apt: u64) {
        ...
        if !AptosCoin::has_balance(&user, amount_of_apt) {
            abort(EInsufficientAptBalance);
        }
    ...
    inline fun check_if_players_are_different(player_one: address, player_two: address) {
        ...
        if player_one == player_two {
            abort(EPlayerAddressesAreTheSame);
        }
    ...
    inline fun check_if_user_is_player(user: &address, game: &Game) {
        ...
        //          EUserNotPlayer
         if user != &game.player_one && user != &game.player_two {
            abort(EUserNotPlayer);
        }
    ...
    inline fun check_if_turn_is_expired(game: &Game) {
        ...
        //          code: ETurnIsExpired
        if game.expiration_timestamp_seconds < Env::get_block_timestamp().into_u64() {
            abort(ETurnIsExpired);
        }
    ...
    inline fun check_if_game_is_expired(expiration_timestamp_seconds: u64) {
        ...
        //          EGameIsNotExpired
        if expiration_timestamp_seconds > Env::get_block_timestamp().into_u64() {
            abort(EGameIsNotExpired);
        }
